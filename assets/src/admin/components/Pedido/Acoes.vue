<template>
    <div>
        ...
        <!-- Imprimir etiqueta -->
        <a v-if="item.status && (item.status == 'released' )" @click="printTicket({id:item.id, order_id:item.order_id})" class="action-button -adicionar" data-tip="Imprimir etiqueta">
        ...
        <!-- Remover etiqueta -->
        <a v-if="item.status && item.order_id && item.id && item.status == 'pending'" @click="removeCart({id:item.id, order_id:item.order_id})" href="javascript:;" class="action-button -excluir" data-tip="Cancelar">
