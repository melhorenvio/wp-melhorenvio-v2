...
const orders = {
    ...
    actions: {
        ...
        insertInvoice: ({ commit }, data) => {
            commit('toggleLoader', true)
            Axios.post(`${ajaxurl}?action=insert_invoice_order&id=${data.id}&number=${data.invoice.number}&key=${data.invoice.key}&_wpnonce=${wpApiSettings.nonce}`).then(response => {
        ...
        addCartSimple: ({ commit }, data) => {
            return new Promise((resolve, reject) => {
                ...
                if (data.id && data.service_id) {
                    Axios.post(`${ajaxurl}?action=add_cart&post_id=${data.id}&service=${data.service_id}&non_commercial=${data.non_commercial}&_wpnonce=${wpApiSettings.nonce}`, data)
                        .then(response => {
                            commit('toggleLoader', false)
        ...
        addCart: ({ commit }, data) => {
            return new Promise((resolve, reject) => {
                ...
                if (data.id && data.service_id) {
                    ...
                    Axios.post(`${ajaxurl}?action=add_order&post_id=${data.id}&service_id=${data.service_id}&non_commercial=${data.non_commercial}&_wpnonce=${wpApiSettings.nonce}`, data)
                        .then(response => {
                            commit('toggleLoader', false)
        ...
        removeCart: (context, data) => {
            context.commit('toggleLoader', true)
            Axios.post(`${ajaxurl}?action=remove_order&id=${data.id}&order_id=${data.order_id}&_wpnonce=${wpApiSettings.nonce}`, data).then(response => {
                if (!response.data.success) {
        ...
        cancelOrder: (context, data) => {
            context.commit('toggleLoader', true)
            Axios.post(`${ajaxurl}?action=cancel_order&post_id=${data.post_id}&order_id=${data.order_id}&_wpnonce=${wpApiSettings.nonce}`, data).then(response => {
                context.commit('setMsgModal', response.data.message)
                context.commit('toggleModal', true)
        ...
        createTicket: ({ commit }, data) => {
            commit('toggleLoader', true)
            Axios.post(`${ajaxurl}?action=print_ticket&id=${data.id}&order_id=${data.order_id}&_wpnonce=${wpApiSettings.nonce}`, data).then(response => {
                if (!response.data.success) {
        ...
        getStatusWooCommerce: ({ commit }) => {
            Axios.get(`${ajaxurl}?action=get_status_woocommerce&_wpnonce=${wpApiSettings.nonce}`).then(response => {
